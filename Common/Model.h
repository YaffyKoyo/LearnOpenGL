...
public:
	...
	int getNumTrianglesOfMesh(int mesh) {
		assert(mesh < meshes.size()); return meshes[mesh].getNumTriangles();
	}
	...
	/*float* getVertex(int mesh, int index) {
		...
		{
			const vector<GLuint> tempTriIndices = meshes[mesh].indices;
			int tmpTriIndex = tempTriIndices[index / 3];
			int tempVertexIndex = 
		}
		else
		{
